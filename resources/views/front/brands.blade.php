...
@section('title', 'Brands Perlengkapan Climbing')
@section('content')
		...
		<section id="Brand" class="flex flex-col gap-[30px] mt-[30px] px-5">
			<div id="PhoneBrands" class="grid grid-cols-2 gap-5">
                ...
                @forelse ($category->brandCategories as $brand)
                    <a href="{{ route('front.brand', $brand->brand->slug) }}" class="flex p-[33px] border border-[#EDEEF0] rounded-2xl overflow-hidden items-center justify-center transition-all duration-300 hover:ring-2 hover:ring-[#FCCF2F]">
                    <div class="h-full w-[100px] flex shrink-0">
                        <img src="{{ Storage::url($brand->brand->logo) }}" alt="brand" class="size-full" />
                    </div>
                    </a> 
                @empty
                    <p>Belum ada brand untuk kategori tersebut</p>
                @endforelse
